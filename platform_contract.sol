...
contract DogezerPlatformContract is Haltable{
    ...
    StandardToken public tokenDGZ;

    string public name = "Dogezer Platform Contract";
    address public dogezerMainAccount;
    mapping(address => uint256) public dgzSpentOf;
    uint public burnPercentage = 2000000000;
    uint public tokenBurned = 0;
    uint public tokensAfterITO = 100000000000000;
    mapping(address => uint256) public licenseLoadedOf;
    ...
    function submitTokens(uint tokenAmount) public
    {
        ...
        require(tokenDGZ.transferFrom(msg.sender, address(0), burnAmount));
        tokenBurned += burnAmount;
        TokenBurn(address(0), burnAmount);

        require(tokenDGZ.transferFrom(msg.sender, dogezerMainAccount, tokenAmount - burnAmount));
        TokenTransfer(dogezerMainAccount, tokenAmount - burnAmount);

        licenseLoadedOf[msg.sender] += licenseAmount;
    ...
    function changeDogezerMainAccount(address _newDogezerMainAccount) public onlyOwner
    {
        if (_newDogezerMainAccount != address(0)) {
            dogezerMainAccount = _newDogezerMainAccount;
        }
    }
